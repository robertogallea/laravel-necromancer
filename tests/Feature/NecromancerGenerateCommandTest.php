...
afterEach(function () {
    ...
    File::delete(config('necromancer.boost.skill_path'));
...
test('when Boost is detected and no --output is given Tier 1 goes to the Boost context path and Tier 2 goes to the Boost skill path', function () {
    ...
    expect(File::exists(config('necromancer.boost.skill_path')))->toBeTrue();
    expect(File::exists(base_path('NECROMANCER.md')))->toBeFalse();
...
test('the overwrite confirmation prompt applies to the Boost skill path (Tier 2)', function () {
    $this->instance(BoostDetector::class, new BoostDetector(ServiceProvider::class));
    $skillPath = (string) config('necromancer.boost.skill_path');
    File::ensureDirectoryExists(dirname($skillPath));
    File::put($skillPath, 'existing content');
    ...
    $this->artisan('necromancer:generate')
        ->expectsConfirmation(basename($skillPath).' already exists. Overwrite?', 'yes')
        ->assertSuccessful();
});

test('the force flag skips the overwrite confirmation for the Boost skill path (Tier 2)', function () {
    $this->instance(BoostDetector::class, new BoostDetector(ServiceProvider::class));
    $skillPath = (string) config('necromancer.boost.skill_path');
    File::ensureDirectoryExists(dirname($skillPath));
    File::put($skillPath, 'existing content');
...
test('when the Boost skill directory cannot be created Tier 2 falls back to NECROMANCER.md with a warning', function () {
    $this->instance(BoostDetector::class, new BoostDetector(ServiceProvider::class));
    config(['necromancer.boost.skill_path' => '/nonexistent-root-dir/skills/necromancer.md']);
